...
class DQN:
    def __init__(self,):
        ...
        #-------------------Agent-------------------
        self.e_greedy=0.8  #0.6
        self.e_greedy_decrement=0.0001
        self.L=200          #Number of training episodes L
    ...
    def main(self,J_num, M_num, O_num, J, Processing_time, D, A):
        ...
        Total_tard=[]
        TR=[]
        for episode in range(self.L):
            Total_reward = 0
            x.append(episode+1)
            print('-----------------------开始第',episode+1,'次训练------------------------------')
            ...
            for i in range(O_num):
                ...
                if done == True:
                    Job_t=Sit.Jobs
                    End_t=[]
                    for Ji_t in range(len(Job_t)):
                        End_t.append(max(Job_t[Ji_t].End))
                    print("max(End_t)", max(End_t))
                    # if (max(End_t) <= 60):
                    #     r_t = r_t + 2
                    # if (max(End_t) <= 58):
                    #     r_t = r_t + 2
                    # if (max(End_t) <= 56):
                    #     r_t = r_t + 4
                    r_t =  r_t + np.exp(63 - max(End_t))/8
                    print("reward per episode: ", r_t)
            ...
            print("Max Completion Time: ", max(End))
            if episode % 10 == 0:
